...
const Outcome = require("../js/llama-score-outcome.js");
...
// principalCountrySet/overlordFor/principalsWithOverlords/principalsForGold/
// principalFieldSum/resolveSideField/principalFieldUnion/reparationsSignal/
// goldLikeLean/revoltOutcomeSignal/economicOutcomeSignal/battleLossSignal/
// shiftConfidence/inferOutcome now live in js/llama-score-outcome.js
// (Outcome, required above) - previously duplicated here and in
// js/llama-score.js as two independently-maintained ~600-line copies that a
// whole-repo review found had already drifted apart in real ways. See that
...
function warSummary(war) {
  ...
  const summary = {
    ...
    // see js/clausewitz.js's extractWarFields and Outcome.inferOutcome().
  ...
  summary.outcome = Outcome.inferOutcome(summary, false, null);
  return summary;
}
  ...
  for (const [warNumber, previous] of Object.entries(previousWars)) {
    if (!current[warNumber]) {
      ...
      events.push({
        ...
        lastWar,
        inferredOutcome: lastWar ? Outcome.inferOutcome(lastWar, true, economy, snapshot.warReparations, economyCountries) : null,
